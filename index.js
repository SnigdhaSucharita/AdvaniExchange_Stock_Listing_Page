...
function sortByPricingAscending(stock1, stock2) {
  return stock1.price - stock2.price;
}

function sortByPricingDescending(stock1, stock2) {
...
app.get("/stocks/sort/pricing", (req, res) => {
  ...
  if(pricing.toLowerCase() === "low-to-high") {
    let sortedStocks = stocksCopy.sort(sortByPricingAscending);
    res.json({ stocks: sortedStocks });
  } else {
    let sortedStocks = stocksCopy.sort(sortByPricingDescending);
    res.json({ stocks: sortedStocks });
  }
})
...
app.get("/stocks/sort/growth", (req, res) => {
  ...
  if(growth.toLowerCase() === "low-to-high") {
    let sortedStocks = stocksCopy.sort(sortByGrowthAscending);
    res.json({ stocks: sortedStocks });
  } else {
    let sortedStocks = stocksCopy.sort(sortByGrowthDescending);
    res.json({ stocks: sortedStocks });
  }
})
